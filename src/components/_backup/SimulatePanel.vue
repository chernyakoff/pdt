<template>
  <q-list style="max-width: 350px;background-color: white;" v-if="dev">
    <q-item v-for="code in products" v-bind:code="code" v-bind:key="code">
      <q-item-section>
        <q-input :model-value="code" />
      </q-item-section>
      <q-item-section side>
        <q-btn flat dense :icon="getIcon('product')" @click="store.resolveCode(code)" />
      </q-item-section>
    </q-item>
    <q-item v-for="code in cells" v-bind:code="code" v-bind:key="code">
      ...
      <q-item-section side>
        <q-btn flat dense :icon="getIcon('cell')" @click="store.resolveCode(code)" />
      </q-item-section>
    </q-item>
  </q-list>
</template >
